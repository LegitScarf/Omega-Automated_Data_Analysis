...
# Modern styling with white header
st.markdown("""
<style>
    /* Import modern fonts */
    ...
    .main-header {
        ...
        color: #1a1a1a !important;
        box-shadow: 0 8px 32px rgba(0, 0, 0, 0.08), 0 0 0 1px rgba(0, 0, 0, 0.05) !important;
    ...
    .main-header::before {
        ...
        right: 0 !important;
        bottom: 0 !important;
        background: radial-gradient(circle at 20% 80%, rgba(26, 26, 26, 0.02) 0%, transparent 50%),
                    radial-gradient(circle at 80% 20%, rgba(0, 0, 0, 0.03) 0%, transparent 50%) !important;
...
export OPENAI_API_KEY="your-api-key-here"
        ...
        try:
            client = OpenAI(
                api_key=api_key,
                timeout=90.0,  # Increased timeout significantly
...
"""
        ...
        response = client.chat.completions.create(
            ...
            max_tokens=5,
            temperature=0,
            timeout=30.0  # Reduced timeout for faster validation
        )
...
"""
        ...
        response = client.chat.completions.create(
            ...
            max_tokens=1200,
            temperature=0.2,
            timeout=45.0  # Balanced timeout for code generation
        )
...
def insights_agent(image_bytes, query, client):
    """Insights Agent - generates insights from visualization with retry mechanism"""
    max_retries = 3
    for attempt in range(max_retries):
        try:
            ...
            response = client.chat.completions.create(
                ...
                max_tokens=1000,
                timeout=60.0  # Longer timeout for vision model
            )
            ...
            if attempt < max_retries - 1:
                st.warning(f"Retry {attempt + 1}/{max_retries} for insights generation...")
                continue
            else:
                return f"Error generating insights after {max_retries} attempts: {str(e)}"

def process_query(data, query, client):
    """Central controller for processing user queries with improved error handling"""
    try:
...
def main():
    # Modern white header with black text at the top
    st.markdown("""
